...
exports.formatData = function(data, oldData){
    ...
    _.forEach(data, function(chunk){
        ...
        if(!chunk.serialNumber){ //if no serialNumber means c5 device is lost
...
exports.profile = function(oauthData){
    var deferred = Q.defer();
    var fetchDays = oauthData.days ? oauthData.days : 1;
    var entries = oauthData.entries ? oauthData.entries : false;
    var lastSyncTime = getLastSyncDate(oauthData);
    var response = {};
    var dateRange = {
      startDate : oauthData.firstDate,
      endDate : oauthData.secondDate
    }
    ...
    if(!_.isEmpty(oauthData.oxy_flow)){
      response['Oxygen Flow Rate (lpm)'] = filterOxygenFlowRate(oauthData.oxy_flow, fetchDays, entries, dateRange);
    }

    if(!_.isEmpty(oauthData.oxy_purity)){
      response['Oxygen Purity (%)'] = filterOxygenPurity(oauthData.oxy_purity, fetchDays, entries, dateRange);
    }

    if(!_.isEmpty(oauthData.hours)){
      response['hours'] = filterHours(oauthData.hours, fetchDays, entries, dateRange);
    }

    if(!_.isEmpty(oauthData.alerts)){
        var alerts = filterAlerts(oauthData.alerts, fetchDays, entries, dateRange);
...
var filterData = function(data, fetchDays, allowPrecisionValue, entries, hours, dateRange){
    ...
    data.forEach(function(val){
        ...
        if(dateRange && dateRange.startDate && dateRange.endDate && newTmpKey <= dateRange.endDate && newTmpKey >= dateRange.startDate){
          returns[units.getFormattedDateTime(val['endDate'])]= val['quantity'];
        }
        else if(fetchDays == 1 && dateRange.startDate == 0 && newTmpKey <= moment().format('YYYY-MM-DD') && newTmpKey >= moment().format('YYYY-MM-DD')){
          returns[units.getFormattedDateTime(val['endDate'])]= val['quantity'];
        }
        else if(fetchDays != 1 && dateRange.startDate == 0 && newTmpKey <= moment().format('YYYY-MM-DD') && newTmpKey >= moment().subtract(fetchDays,'days').format('YYYY-MM-DD')){
            returns[units.getFormattedDateTime(val['endDate'])]= val['quantity'];
        };
    ...
    if(hours == 'hours'){
      _.forEach(returns, function(reading, date){
        if(dateRange && dateRange.startDate !=0){
          returns[moment(dateRange.startDate).format('D MMM YYYY 00:00:00')] = 0;
        }
        else if(moment(date).format('YYYY-MM-DD') != moment().subtract(fetchDays,'days').format('YYYY-MM-DD')){
          returns[moment().subtract(fetchDays,'days').format('D MMM YYYY 00:00:00')] = 0;
        }
      });
    }
    return returns;
...
var filterAlertsData = function(data, fetchDays, entries, dateRange){
    if(data === undefined) return false;
    var returns = {};
    var days = [];
    var response = {};
...
var filterOxygenFlowRate = function(data, fetchDays, entries, dateRange){
    data = _.uniqBy(data, 'endDate'); // remove duplicate entries
    return filterData(data, fetchDays, true, entries, 'oxy-flow', dateRange);
};

//filter Oxygen puriy data from Oxygen app
var filterOxygenPurity = function(data, fetchDays, entries, dateRange){
    data = _.uniqBy(data, 'endDate'); // remove duplicate entries
    return filterData(data, fetchDays, true, entries, 'oxy-purity', dateRange);
};
//filter no of hours used data form Oxygen app
var filterHours = function(data, fetchDays, entries, dateRange){
    data = _.uniqBy(data, 'endDate'); // remove duplicate entries
    return filterData(data, fetchDays, true, entries , 'hours', dateRange);
};
//filter alerts form Oxygen app
var filterAlerts = function(data, fetchDays, entries, dateRange){
    data = _.uniqBy(data, 'endDate'); // remove duplicate entries
    return filterAlertsData(data, fetchDays, entries, dateRange);
};
